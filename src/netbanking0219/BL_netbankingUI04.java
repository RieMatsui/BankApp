...
public class BL_netbankingUI04 {
	...
	public boolean checkNull(DTO_netbankingUI04 _paramData) {
		...
		//パスワード未入力時
		if (_paramData.getStr_userPW().isEmpty()) {
		...
		//確認パスワード未入力時
		else if (_paramData.getStr_userPW().isEmpty()) {
	...
	public boolean cfmSamePw(DTO_netbankingUI04 _paramData) {
		...
		System.out.println(_paramData.getStr_userPW());
		System.out.println(_paramData.getStr_pwCfm());


		if (!_paramData.getStr_userPW().equals(_paramData.getStr_pwCfm())){
	...
	public boolean createErrorData(DTO_netbankingUI04 _paramData) {
		System.out.println("エラーテーブルの作成");
		int res = 0;
		...
		try {
			...
			res = vrb_abs.insertErrorTable(vrb_con, _paramData);
	 ...
	 *
	 */
	public boolean inputPasswordData(DTO_netbankingUI04 _paramData) {
		System.out.println("TEMPユーザにPWのアップデート");
		int res = 0;
		JDBCAbstractTemplate vrb_abs = new JDBCTmplMySQL();
		Connection vrb_con;
		...
		try {
			vrb_con = vrb_abs.dbConection();
			res	= vrb_abs.updateTmpUserPassword(vrb_con, _paramData.getStr_pwCfm(),_paramData.getNum_userId());

			if (res == 0) {
				JOptionPane.showMessageDialog(null,
	                    "TESTMESSAGE>予期せぬエラーが発生しました",
	                    "TITLE>エラー",
	                    JOptionPane.INFORMATION_MESSAGE);
				return false;
			}
	...
	public boolean checkAvailableInfomation(DTO_netbankingUI04 _paramDto) {
		...
		if (!inputPasswordData(_paramDto)) {
